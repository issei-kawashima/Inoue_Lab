...
!2019.12.18 超音速噴流の流入を加えた二次元コード
! 撹乱入れない。流入部は超音速ジェットのdirichlet条件、x方向流出部はNeumann条件を適用する。NSCBC_xのsubroutineは削除
! outfolw subroutineの中のx関係の式も全て削除した。
! Pr=1, Ma=2.4に変更した。M=600程度でNan
! Ma=1.4でt=100まで回った
! Ma=2で計算。uを出力するように変更M=920でNan。Ma=0.7と比較してもrhoの振幅が大きいのでNx=180>270, Ny=100>150へと変更した。
! Nx,Nyを最大2倍づつにしたがM=562程度でNanになってしまうので、dtの問題と考えた。
! Nx,Nyを初期(180,100)に戻して試行。dt=5.d-3からdt=2.5d-3へ変更。M=1839つまりほとんど同じところでNaN。
!2019.12.19 Bufferを流入部にもつけて、Nx-180,Ny=100,dt=5.d-3でMa=2.4で計算してみた。計算できたけど流入してくれない
!!!!!!!メインの計算はyobi.f90で行って、M1の中間報告を出している。!!!!!!!!!!!
!2020.05.09 Bufferは流出部にのみ設置してある。Pr=1, Ma=2.4, Nx=180, Ny=100, dt=5.d-3, Tjet=1.12Tempで計算したらt=100まで計算できた。
...
!速度勾配テンソルの第二不変量Qを実装。Ma=1.6にしてみて計算してみる。=>ダメだった。
!Ma=1.5ならどうか？
...
!==============================================================================
    ...
    subroutine outflow(UVT,dUVTy,Vy,dVy)
      ...
      !境界のdVyに条件を設定するのでdVの計算ができた後に境界値のみ上書きをする
      !y方向左側の条件設定
      dVy(1,:,0) = 0.d0
    ...
    subroutine buffer_x(c_infty,Ux,sigma_x,zeta_fx)
      ...
      enddo
!もしxの境界も両方流出にしたい場合は以下のようにBufferを設置する
!      if((x1 >= Xmin).and.(x1<(Wlx+Xmin))) then
!        sigma_x(:,i,:) = alpha_sigma*c_infty*((-x1+Xmin+Wlx)/Wlx)**3.d0
!      elseif((x1>=(Wlx+Xmin)).and.(x1<(Xmax-Wrx))) then
!        sigma_x(:,i,:) = 0.d0
!      elseif((x1>=(Xmax-Wrx)).and.(x1<=Xmax)) then
!        sigma_x(:,i,:) = alpha_sigma*c_infty*((x1-(Xmax-Wrx))/Wrx)**3.d0
!      endif
!    enddo
...
!一応ゼロクリア
    ...
    !x=0の軸上にのみ流入条件を適用することでここからどんどん流入が起こる
      ...
      G(0,:,:) = 1.d0!ρ
      G(1,:,:) = 0.d0!u
      !あらかじめジェットを流れ場全体に設定することでFirst Schockを無くす。
!      do i=0,Nx
!        G(0,i,:) = in_G(0,:)!ρ
!        G(1,i,:) = in_G(1,:)!u
!      enddo
