...
!それにより、para viewで3次元での可視化を可能にするファイルが生成される
program main
implicit none
integer,parameter :: Nx = 180+1 !#最後のMを出力結果から削除した最新版ではこちら(一行改行分を含めてる)
! integer,parameter :: Nx = 180+2 !#最後にMが入っている旧バージョンでの計算結果はこちら。Nx+1にして#Mのせい読み込みがでずれないようにする
