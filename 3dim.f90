...
!2020.05.28 周期条件のプログラムそのものは正しいとして、流入条件がz方向の全ての箇所に適用されているため。周期条件が上手く働かないのではないか？
!そこで。矩型JetをZ方向 k=8~10の部分にのみ流入させるように流入条件を変更
!2020.05.28 流入条件にかかわらずM=307でNanとなった。
!2020.05.29森山の助言に従って、2次元の結果と比較することにした。計算条件が同じで流入条件をz方向全てに適用しれば計算結果は全く同じになるはずだから。
!2020.05.29 角処理を1/3にしたものでz=0でM=306(計算破綻直前)のrhoを比較したら
