...
!Pr=0.71>Pr=1へ変更。Ma数を0.5から1.4に変更(超音速化)。また周期的撹乱も加えた。2次元では計算できた条件。
...
!      !Doループ内でin_G(2,:,:)をDirichlet条件で設定して撹乱を導入しているのでここでは設定しない
      ...
      !もちろん出力もζ_y座標系とζ_x座標系で行う
           do k=0,Nz-1
             z = dz*dble(k)
             write(z_name, '(i2.2)') k
             open(10, file = "result_test_super/parameter000000_"//trim(z_name)//".txt")
              ! z = dz*dble(Nz/2)
              do i = 0,Ny
                do j = 0,Nx
              ...
              close(10)
           enddo
!      open(20,file = "result_test_super/1pressure.d")
...
!        endif
         ...
         call dif_z(ccs_sigma,dz,G,dGz,LUccsz)
           ...
           do kk= 0,Nz-1
             ...
             open(10, file = "result_test_super/parameter"//trim(filename)//"_"//trim(z_name)//".txt")
             z=dz*dble(kk)
             ! z=dz*dble(Nz/2)
             do ii = 0,Ny
               do jj = 0,Nx
