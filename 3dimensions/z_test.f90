...
!一応ゼロクリア
      ...
      !=========================================================================
      !計算条件を振り返れるように書き出す
      open(50, file = "result_z_test/Conditon_list.csv")
  ...
  !!!!!!============流入条件設定==================================================
    ...
    !==============================流入ランダム撹乱の読み込み========================
      ...
      ! open(34,file='result_z_test/kakkuran_kakunin.txt',status='replace')
      ! do k=0,Nz
      !   do i=0,Ny
...
!=====大きいDoループなので並列化する=================================================
  ...
  !$omp parallel sections
    !$omp section
       do k=0,Nz
         write(z_name, '(i2.2)') k
         open(10, file = "result_z_test/parameter000000_"//trim(z_name)//".txt")
          do i = 0,Ny
            do j = 0,Nx
    ...
    !$omp end parallel do
      ...
      !=======ファイルへの書き出しはもちろん順番が大切なので、並列化不可能====================
           do kk=0,Nz
             ...
             open(10, file = "result_z_test/parameter"//trim(filename)//"_"//trim(z_name)//".txt")
             do ii = 0,Ny
               do jj = 0,Nx
                 write(10,'(f24.16,",",f24.16,",",f24.16,",",f24.16,",",f24.16,",",&
...
!              Q = Q1!オイラー法の時間の更新
      ...
      enddo DNS
! ===========メイン計算終了========================================================
    open(41, file = "result_z_test/turbulent_check_1.csv")
    open(42, file = "result_z_test/turbulent_check_2.csv")
    open(43, file = "result_z_test/turbulent_check_3.csv")
    open(44, file = "result_z_test/turbulent_check_4.csv")
