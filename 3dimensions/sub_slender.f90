...
!Nxを変える => BufferのUxを0にする範囲と(一応)乱流チェックポイントの座標を変える
!Ny,Nzを変える => random_3DとBufferのUxを0にする範囲と(一応)乱流チェックポイントの座標を変える
...
!2020.12.2 inflow subroutineで矩形ジェットを流入させない区間に関しては以前のエラーが超音速で起きていた状態の名にも適用しないものに戻してみた
...
!一応ゼロクリア
      ...
      enddo
      !基本デカすぎのz_tempoだが、一部だけ正しい値kが入った。
