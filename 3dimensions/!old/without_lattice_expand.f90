...
!2020.05.21 前回(5/13)に計算したときはBuffer_yのy座標の設定方法が間違えていたので、今回はそこも修正した。
!2020.05.21 M=491でNan.計算が破綻する直前を見るとx=Nx, y=Nyの点でrhoがおかしくなっている。これは今回書き換えていない場所の境界条件なので
!元々のコードの問題な気がする.これは大発見なのでは？？
!2020.05.22 NSCBC_xの左側流出条件がy方向の物をコピペしたのでuがvになっていた。>修正済み。
!2020.05.22 5/21の条件で計算し直し。M=491でNan.変換なし。
...
module threedim
  ...
  !!!x方向も軸対象座標に変更するので、Lxの定義は変更なし.値自体はLx=36から18に変更。18*2=36で長さ自体は一緒。
...
!呼び出して計算しなければいけないので今回はQから直接計算できるようにプログラムを組んだ
  ...
  !DCS右辺の計算(RHS)サブルーチン
  !x方向
  subroutine dif_x(sigma,dx,Fx,x,LU)
      integer i
  ...
  endsubroutine dif_x
   !DCS右辺の計算(RHS)サブルーチン
   !y方向
  subroutine dif_y(sigma,dy,Fy,x,LU)
       integer i
  ...
  subroutine NSCBC_x(G,dGx,dFx,pNx_infty,p0x_infty)
    ! subroutine NSCBC_x(G,dGx,dFx,pNx_infty)
      ...
      double precision pNx_infty,p0x_infty!(4方向流出のため追加)
      ...
      !=========================================================================
      !4方向流出のため追加
      !x方向右側つまりi=0の点において無反射流出条件でL行列を設定する
      L(1,:,:,0) = (G(1,0,:,:) - c_NS(:,:,0)) * (-G(0,0,:,:)*c_NS(:,:,0)*dGx(1,0,:,:)+dGx(4,0,:,:))
      L(2,:,:,0) = G(1,0,:,:) * ((c_NS(:,:,0) ** 2.d0)*dGx(0,0,:,:) - dGx(4,0,:,:))
      L(3,:,:,0) = G(1,0,:,:) * dGx(2,0,:,:)
      L(4,:,:,0) = G(1,0,:,:) * dGx(3,0,:,:)
  ...
  endsubroutine NSCBC_y
    ...
    subroutine outflow(UVWT,dUVWTx,Vx,dVx,dUVWTy,Vy,dVy)
      ...
      !4方向流出のために追加
      !x方向左側の条件設定
      dVx(2,0,:,:) = 0.d0!dτ12/dx
      dVx(3,0,:,:) = 0.d0!dτ13/dx
      dVx(4,0,:,:) = dVx(1,0,:,:)*UVWT(1,0,:,:)+dUVWTx(1,0,:,:)*Vx(1,0,:,:)&
                    +dUVWTx(2,0,:,:)*Vx(2,0,:,:)+dUVWTx(3,0,:,:)*Vx(3,0,:,:)
      !=========================================================================
      !x方向右側の条件設定
    ...
    subroutine buffer_x(c_infty,Ux,sigma_x,dx)
      integer i
      double precision c_infty,Xmax,Xmin,x1,dx
      ...
      do i = 0,Nx
        ...
        if(x1<(Wlx+Xmin)) then
          sigma_x(:,i,:,:) = alpha_sigma*c_infty*((-x1+Xmin+Wlx)/Wlx)**3.d0
        elseif((x1>=(Wlx+Xmin)).and.(x1<(Xmax-Wrx))) then
          sigma_x(:,i,:,:) = 0.d0
        elseif(x1>=(Xmax-Wrx)) then
          sigma_x(:,i,:,:) = alpha_sigma*c_infty*((x1-(Xmax-Wrx))/Wrx)**3.d0
        endif
      enddo
       !dx不要かも？
       !========================================================================
       !Uxは片側のみにある必要はないので今回は0クリアしない(4方向流出)
       !========================================================================
     ! Ux(:,0:90,:,:) = 0.d0!x左側のBufferを取るためにWlxの範囲のUxを確実に0に設定している
      ! open(100,file="ux-check.csv")
      ! do i=0,Nx
      !   x1 = -Lx + dx*dble(i)
      !   write(100,*) x1,",",Ux(0,i,0,0),",",sigma_x(0,i,0,0)
    ...
    !y方向
    subroutine buffer_y(c_infty,Uy,sigma_y,dy)
      integer i
      double precision Ymax,Ymin,c_infty,y1,dy
    ...
    program main
      ...
      double precision  dx,x,pNx_infty,p0x_infty
      !y方向
...
!      call lattice_x(dx,zeta_fx,dzeta_inx)
    ...
    do i = (Ny/2)+1,Ny
      ...
      ur(i) = ujet/2.d0*(1.d0 - dtanh((12.5d0/4.d0)*((y/b)- (b/y))))
    ...
    !初期値の設定
      ...
      call buffer_x(c_infty,Ux,sigma_x,dx)
      call buffer_y(c_infty,Uy,sigma_y,dy)
    !流入条件
      ...
      do k=0,Nz-1
        ...
        do i = 0,Ny
          ...
          do j = 0,Nx
            ...
            f24.16)') x,y,z,G(0,j,i,k),omega_3(j,i,k),dp(j,i,k)/dt
          enddo
          write(10,*)
        enddo
        close(10)
...
!      write(20,'(1I1,1f24.16)') 0,G(3,162,Ny/2,Nz/2)!(23,7)を指定しているが実際は(22.89,6.97)にずれてしまう
      !p_inftyの定義
      p0x_infty = G(4,0,0,0)!4方向流出のため設定
...
!        enddo
        ...
        call dif_x(psigma,dx,Fpx,xp,LUpx)
        call dif_x(msigma,dx,Fmx,xm,LUmx)
        !y_axis dFy/dyの計算
        call dif_y(psigma,dy,Fpy,yp,LUpy)
        call dif_y(msigma,dy,Fmy,ym,LUmy)
        !z_axis
        ...
        !x_axis dUVWT/dxの計算
        call dif_x(ccs_sigma,dx,UVWT,dUVWTx,LUccsx)
        !y_axis dUVWT/dyの計算
        call dif_y(ccs_sigma,dy,UVWT,dUVWTy,LUccsy)
        !z_axis dUVWT/dzの計算
        ...
        !x_axis
        call dif_x(ccs_sigma,dx,Vx,dVx,LUccsx)
        !y_axis
        call dif_y(ccs_sigma,dy,Vy,dVy,LUccsy)
        !z_axis
        call dif_z(ccs_sigma,dz,Vz,dVz,LUccsz)
        !NSCBCの計算開始
        !x方向のNSCBCの計算
        call dif_x(ccs_sigma,dx,G,dGx,LUccsx)
        ! call NSCBC_x(G,dGx,dFx,pNx_infty)
        !=======================================================================
        !4方向流出のため追加
        call NSCBC_x(G,dGx,dFx,pNx_infty,p0x_infty)
        !=======================================================================
        !y方向
        call dif_y(ccs_sigma,dy,G,dGy,LUccsy)
        ...
        call dif_x(ccs_sigma,dx,Q,dQx,LUccsx)
        call dif_y(ccs_sigma,dy,Q,dQy,LUccsy)
...
!!      call Neumann(Q1)
      ...
      !適用しなければならない
        ...
        call outflow(UVWT,dUVWTx,Vx,dVx,dUVWTy,Vy,dVy)
        !Buffer領域の計算
        call dif_x(ccs_sigma,dx,Q1,dQx,LUccsx)
        call dif_y(ccs_sigma,dy,Q1,dQy,LUccsy)
...
!        call Neumann(Q2)
      ...
      !適用しなければならない
        ...
        call outflow(UVWT,dUVWTx,Vx,dVx,dUVWTy,Vy,dVy)
        !Buffer領域の計算
        call dif_x(ccs_sigma,dx,Q2,dQx,LUccsx)
        call dif_y(ccs_sigma,dy,Q2,dQy,LUccsy)
           ...
           z=dz*dble(kk)
             do ii = 0,Ny
               ...
               do jj = 0,Nx
                 ...
                 write(10,'(f24.16,",",f24.16,",",f24.16,",",f24.16,",",f24.16,",",&
                 f24.16)') x,y,z,G(0,jj,ii,kk),omega_3(jj,ii,kk),dp(jj,ii,kk)
               enddo
               write(10,*)
             enddo
           close(10)
...
!                  if(isnan(Q1(0,i,j,k))) then
                    ...
                    call rho_u_p(oldG,Q)
                    call dif_x(ccs_sigma,dx,oldG,dGx,LUccsx)
                    call dif_y(ccs_sigma,dy,oldG,dGy,LUccsy)
                    ...
                    do kk= 0,Nz-1
                      ...
                      do ii = 0,Ny
                        ...
                        do jj = 0,Nx
                          ! x = dx * dble(jj)
                          x = -Lx + dx * dble(jj)!4方向流出
                          write(10,'(f24.16,",",f24.16,",",f24.16,",",f24.16,","&
                          ,f24.16)') x,y,z,oldG(0,jj,ii,kk),omega_3(jj,ii,kk)
                        enddo
                        write(10,*)
