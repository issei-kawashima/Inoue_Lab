...
program kakusan
  ...
  double precision,dimension(0:NZ) :: z,zs
  double precision,dimension(0:NY,0:NZ)::u_d3,v_d3,w_d3
...
!!!初期座標および格子伸長
  ...
  max_E=maxval(ES3d)
  open(120,file='dirturbance_conditions_NPB/E(k)_check.csv')
  !エネルギースペクトルを確認
  do Kz=1,Kmz
    do Ky=1,Kmy
       do Kx=1,Kmx
                 ...
                 !0<=Rand_x<=1の範囲でランダムに値を与える
                  ...
                  u_d3(j,k)=u_d3(j,k)&
                   +dsqrt(ES3D(Kx,Ky,Kz))*dsin(2d0*pi*Kx*(0.d0+theta_x)/Lx)&
                       ...
                       *dsin(2d0*pi*Kz*(zs(k)+theta_z)/Lz)
               end do
            end do
         end do
      end do
...
!$omp end parallel sections
 ...
 open(21,file='dirturbance_conditions_NPB/kakuran3D_u.txt',status='replace')
 open(22,file='dirturbance_conditions_NPB/kakuran3D_v.txt',status='replace')
 open(23,file='dirturbance_conditions_NPB/kakuran3D_w.txt',status='replace')
 ...
 open(100,file='dirturbance_conditions_NPB/check.csv')
 !u方向の撹乱関数を確認
do k=0,NZ
   do j=0,NY
